...
<div class="card">
        ...
        <div class="card-header row gutters-5">
            ...
            <div class="col-md-2">
                <select class="form-control form-control-xs aiz-selectpicker" name="per_page" onchange="sort_uploads()">
                    ...
                    <option value="1000" @if($perPage==1000) selected="" @endif>{{ translate('1000 per page') }}
                    </option>
                </select>
            </div>
            <div class="col-md-2">
    ...
    <div class="card-body">
        <div class="row gutters-5">
            ...
            $is_missing = !$file_exists;
            @endphp
            <div class="col-auto w-140px w-lg-220px" style="position:relative;">
                <div class="file-checkbox-overlay"
                    style="top: 0px !important; position: absolute; z-index: 9999 !important;">
                    <input type="checkbox" class="file-checkbox" value="{{ $file->id }}"
                        data-name="{{ $file_name }}.{{ $file->extension }}" autocomplete="off">
                </div>
                <div class="aiz-file-box @if($is_missing) is-missing @endif" data-file-id="{{ $file->id }}">
                    <div class="dropdown-file">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            ...
                            @if($file_exists)
                            <a href="{{ $file_url }}" target="_blank" download="{{ $file_name }}.{{ $file->extension }}"
                                class="dropdown-item">
    ...
    <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                ...
                <button type="button" class="btn btn-link mt-2" data-dismiss="modal">{{ translate('Cancel') }}</button>
                <button type="button" class="btn btn-danger mt-2" id="bulk-delete-confirm">{{ translate('Delete All')
                    }}</button>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    function detailsInfo(e){
        ...
        // Bulk action logic v3 - simple, no label interference
        (function(){
            ...
            // Click on the card body (not on dropdown/checkbox) toggles selection
            ...
            // Direct change on the checkbox (e.g. clicking it directly)
            $(document).on('change', '.file-checkbox', refreshUI);
            // Prevent card click handler from double-firing when clicking the checkbox itself
            $(document).on('click', '.file-checkbox-overlay', function(e){ e.stopPropagation(); });

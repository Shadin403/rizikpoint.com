...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        // category_id can be a top-level category; include its sub-categories too
        if ($request->filled('category_id') || $request->filled('category')) {
            $catId = (int) ($request->query('category_id') ?? $request->query('category'));
            if ($catId > 0) {
                $ids = CategoryUtility::children_ids($catId);
                $ids[] = $catId;
        ...
        // seller_id / storeId filter
        if ($request->filled('seller_id') || $request->filled('storeId')) {
            $sellerId = (int) ($request->query('seller_id') ?? $request->query('storeId'));
            if ($sellerId > 0) {
        ...
        return new ProductMiniCollection($query->latest()->paginate($perPage));
    }
    ...
    {
        $product = Product::where('id', $request->id)->orWhere('slug', $request->id)->firstOrFail();
        $str = '';
        $tax = 0;
        ...
        $product_stock = $product->stocks->where('variant', $str)->first() ?: $product->stocks->first();
        $price = $product_stock ? $product_stock->price : $product->unit_price;
        $stockQuantity = $product_stock ? $product_stock->qty : 0;

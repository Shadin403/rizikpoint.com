				...
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group row">
						...
						<div class="col-md-8">
							...
							<input type="text" name="app_name" class="form-control" placeholder="{{ translate('App Name') }}" value="{{ get_setting('app_name') }}">
						</div>
					</div>
                    <div class="form-group row">
                    ...
                    <div class="border-top pt-3">
                        <div class="form-group row">
                            <label class="col-md-3 col-from-label">{{translate('Header Marquee Text (English)')}}</label>
                            <div class="col-md-8">
                                <div class="form-group">
                                    <input type="hidden" name="types[]" value="header_marquee_text_en">
                                    <textarea class="form-control" name="header_marquee_text_en" rows="4" placeholder="{{ translate('Enter marquee text for English version. Put each message on a new line.') }}">{{ get_setting('header_marquee_text_en') }}</textarea>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-3 col-from-label">{{translate('Header Marquee Text (Bengali)')}}</label>
                            <div class="col-md-8">
                                <div class="form-group">
                                    <input type="hidden" name="types[]" value="header_marquee_text_bn">
                                    <textarea class="form-control" name="header_marquee_text_bn" rows="4" placeholder="{{ translate('Enter marquee text for Bengali version. Put each message on a new line.') }}">{{ get_setting('header_marquee_text_bn') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>

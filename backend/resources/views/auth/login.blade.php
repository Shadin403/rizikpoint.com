...
<div class="login-split-container">
    ...
    <div class="login-visual-panel">
        <div class="visual-bg-layer layer-1 active" style="background-image: url('{{ $random_bg }}')"></div>
        ...
        <div class="login-visual-content">
            <div class="login-visual-header">
                <div>
                    @if(get_setting('system_logo_white') != null)
                        <img src="{{ uploaded_asset(get_setting('system_logo_white')) }}" height="40" style="max-height: 40px; object-fit: contain;">
                    @elseif(get_setting('system_logo_black') != null)
                        <img src="{{ uploaded_asset(get_setting('system_logo_black')) }}" height="40" style="max-height: 40px; object-fit: contain; filter: brightness(0) invert(1);">
                    @else
                        <img src="{{ static_asset('assets/img/logo.png') }}" height="40" style="max-height: 40px; object-fit: contain; filter: brightness(0) invert(1);">
                    @endif
                </div>
                <a href="{{ route('home') }}" class="btn-back-web">
            ...
            <div class="login-tagline-wrap">
                ...
                <div class="login-carousel-dots">
                    @foreach($nature_images as $index => $img)
                        <span class="login-dot {{ $random_bg === $img ? 'active' : '' }}" data-index="{{ $index }}"></span>
                    @endforeach
                </div>
            </div>
        </div>
    ...
    <div class="login-form-panel">
        <div class="login-form-wrap">
            ...
            <div class="mobile-logo-wrap">
                @if(get_setting('system_logo_white') != null)
                    <img src="{{ uploaded_asset(get_setting('system_logo_white')) }}" class="mx-auto mb-4" height="40" style="max-height: 40px; object-fit: contain;">
                @elseif(get_setting('system_logo_black') != null)
                    <img src="{{ uploaded_asset(get_setting('system_logo_black')) }}" class="mx-auto mb-4" height="40" style="max-height: 40px; object-fit: contain; filter: brightness(0) invert(1);">
                @else
                    <img src="{{ static_asset('assets/img/logo.png') }}" class="mx-auto mb-4" height="40" style="max-height: 40px; object-fit: contain; filter: brightness(0) invert(1);">
                @endif
            </div>

            <div class="mb-4 text-center text-lg-left">
                <h1 class="h3 text-white font-weight-bold mb-2">{{ translate('Login to account') }}</h1>
                <p class="text-muted mb-0" style="font-size: 13.5px; opacity: 0.8;">{{ translate('Welcome back! Please enter your details.') }}</p>
            </div>
            
            <form class="pad-hor" method="POST" role="form" action="{{ route('login') }}">
                @csrf
                <div class="form-group form-group-custom">
                    <input id="email" type="email" class="form-control input-custom {{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required autofocus placeholder="{{ translate('Email') }}">
                    @if ($errors->has('email'))
                ...
                <div class="form-group form-group-custom">
                    <input id="password" type="password" class="form-control input-custom {{ $errors->has('password') ? ' is-invalid' : '' }}" name="password" required placeholder="{{ translate('Password') }}">
                ...
                <div class="row align-items-center mb-4">
                    ...
                    @if(env('MAIL_USERNAME') != null && env('MAIL_PASSWORD') != null)
                        <div class="col-6 text-right">
                            <a href="{{ route('password.request') }}" class="text-reset-custom">{{ translate('Forgot password ?') }}</a>
                        </div>
                    @endif
                </div>
                        ...
                        <tbody>
                            <tr class="align-middle">
                                ...
                                <td class="pr-0 text-right">
                                    <button class="btn btn-soft-primary btn-xs font-weight-bold" style="border-radius: 6px;" onclick="autoFill()">{{ translate('Copy') }}</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
@section('script')
    <script type="text/javascript">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const bgImages = @json($nature_images);
            ...
            let activeLayer = 1;
            let currentIndex = bgImages.indexOf('{{ $random_bg }}');

...
// Remove the OLD broken rules and append fresh, correct ones
$css = preg_replace(
    "/\/\* Bulk delete UI - admin uploaded files[\s\S]*?\.file-checkbox-card\.is-missing::after\{[\s\S]*?\}\s*/u",
    "",
    $css
);
...
/* Bulk delete UI - admin uploaded files (v3 - clean) */
...
echo "CSS updated. Size: ".filesize($path)."\n";

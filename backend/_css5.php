...
// Replace the v4 block
$v4start = strpos($css, "/* Bulk delete UI - admin uploaded files (v4");
if ($v4start !== false) {
    // Find next "/*" comment after v4 - end of v4
    $v4end = strpos($css, "/*", $v4start + 10);
    if ($v4end === false) $v4end = strlen($css);
    $css = substr($css, 0, $v4start) . substr($css, $v4end);
}

$new = <<<CSS
/* Bulk delete UI - admin uploaded files (v5 - final) */
.col-auto[style*="position:relative"]{position:relative !important;}
...
.col-auto[style*="position:relative"].is-selected .file-checkbox-card{border-color:#1890ff;background:#e6f4ff;box-shadow:0 0 0 2px rgba(24,144,255,.25);}
.col-auto[style*="position:relative"].is-selected .file-checkbox-card .card-body h6{color:#1890ff;font-weight:600;}
...
.col-auto[style*="position:relative"]:hover .file-checkbox-overlay{border-color:#1890ff;}
.col-auto[style*="position:relative"].is-selected .file-checkbox-overlay{background:#1890ff;border-color:#1890ff;}

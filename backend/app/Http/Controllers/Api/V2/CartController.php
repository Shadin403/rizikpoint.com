...
class CartController extends Controller
{
            ...
            foreach ($owner_ids as $owner_id) {
                ...
                if (!empty($shop_items_raw_data)) {
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                        ...
                        $stock_obj = $product->stocks->where('variant', $shop_items_raw_data_item['variation'])->first() ?: $product->stocks->first();
                        $shop_items_data_item["upper_limit"] = intval($stock_obj ? $stock_obj->qty : 0) ;
    ...
    public function add(Request $request)
    {
        ...
        if ($variant == '')
            ...
            $product_stock = $product->stocks->where('variant', $variant)->first() ?: $product->stocks->first();
        ...
        $stock_obj = $product->stocks->where('variant', $variant)->first() ?: $product->stocks->first();
        $stock = $stock_obj ? $stock_obj->qty : 0;
    ...
    public function changeQuantity(Request $request)
    {
        ...
        if ($cart != null) {
            ...
            $stock_obj = $cart->product->stocks->where('variant', $cart->variation)->first() ?: $cart->product->stocks->first();
            $qty = $stock_obj ? $stock_obj->qty : 0;
    ...
    public function process(Request $request)
    {
        ...
        if (!empty($cart_ids)) {
            ...
            foreach ($cart_ids as $cart_id) {
                ...
                $stock_obj = $cart_item->product->stocks->where('variant', $cart_item->variation)->first() ?: $cart_item->product->stocks->first();
                $stock = $stock_obj ? $stock_obj->qty : 0;
    ...
    public function sync(Request $request)
    {
        ...
        if ($request->has('cost_matrix') && \App\Utility\NagadUtility::create_balance_reference($request->cost_matrix) == false) {
            return response()->json(['result' => false, 'message' => 'Cost matrix error' ]);
        }
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                // Validate minimum quantity and stock (just like individual add method does)
                if ($product->min_qty > $quantity) {
                    continue; // Skip if minimum quantity condition is not met
                }

                $stock_obj = $product->stocks->where('variant', $variant)->first() ?: $product->stocks->first();
                $stock = $stock_obj ? $stock_obj->qty : 0;
                if ($stock < $quantity) {
                    continue; // Skip if stock is insufficient
                }
            ...
            return response()->json([
                'result' => true,
                'message' => translate('Cart synchronized successfully')
            ], 200);
            ...
            return response()->json([
                'result' => false,
                'message' => 'Cart synchronization failed: ' . $e->getMessage()
            ], 500);
        }
    }

...
class BusinessSettingsController extends Controller
{
    ...
    public function storefront_info(Request $request)
    {
        CoreComponentRepository::instantiateShopRepository();
        CoreComponentRepository::initializeCache();
        return view('backend.setup_configurations.storefront_info');
    }
    ...
    public function delivery_info(Request $request)
    {
        CoreComponentRepository::instantiateShopRepository();
        CoreComponentRepository::initializeCache();
        // One-time: tag any legacy delivery-related key with lang='delivery' so it shows up here.
        BusinessSetting::whereNull('lang')
            ->where(function ($q) {
                $q->where('type', 'like', 'shipping_%')
                  ->orWhere('type', 'like', 'return_%')
                  ->orWhere('type', 'like', 'delivery_%')
                  ->orWhere('type', 'like', 'free_delivery_%');
            })
            ->update(['lang' => 'delivery']);
        return view('backend.setup_configurations.delivery_info');
    }
    ...
    {
        // Dynamic key:value editor (storefront_info / delivery_info pages) — accepts keys[]/values[].
        if ($request->exists('keys') || $request->exists('values') || $request->has('keys') || $request->has('values')) {
            $keys   = (array) $request->input('keys', []);
            $values = (array) $request->input('values', []);
            $builtinTypes = ['site_name','timezone','system_logo_white','system_logo_black','header_marquee_text_en','header_marquee_text_bn'];
            $keptTypes = [];
            foreach ($keys as $i => $rawKey) {
                $key = trim((string) $rawKey);
                if ($key === '') continue;
                if (in_array($key, $builtinTypes, true)) continue; // never overwrite built-ins
                $val = $values[$i] ?? '';
                $keptTypes[] = $key;
                $source = $request->input('source');
                $row = BusinessSetting::where('type', $key)->first();
                if ($row) {
                    $row->value = $val;
                    // Tag the row with which admin page it came from so the frontend
                    // can filter the right keys on the storefront.
                    if ($source === 'delivery_info') {
                        $row->lang = 'delivery';
                    } elseif ($source === 'storefront_info') {
                        $row->lang = 'storefront';
                    }
                } else {
            ...
            Artisan::call('cache:clear');
            Cache::forget('api:v2:business_settings');
            flash(translate('Storefront info updated successfully'))->success();
            return back();
        }
    ...
    /**
     * Invalidate the V2 business-settings API cache when an admin
     * updates any setting. Falls back to artisan cache:clear for the
     * cases that are not routed through update()/updateActivationSettings().
     */
    private function bustBusinessSettingsApiCache(): void
    {
        Cache::forget('api:v2:business_settings');
        Artisan::call('cache:clear');
    }
    ...
    {
        // no-op; placeholder for future middleware hooks
    }

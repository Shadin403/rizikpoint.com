...
const cartItem = computed(() => {
  ...
  return items.value.find(i => i.id === deal.value.id);
});
const cartQty = computed(() => {
  return cartItem.value ? cartItem.value.quantity : 0;
});
const resolvedMainImage = computed(() => {
  if (!deal.value?.imageUrl) return null;
  const url = deal.value.imageUrl;
...
function handleIncrease(event) {
  ...
  if (event && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    animateFlyToCart(event, resolvedMainImage.value || deal.value?.imageUrl);
  }
}
...
<template>
  <div class="pb-10 bg-gray-50 min-h-screen overflow-x-hidden">
    ...
    <!-- Product Detail -->
    <div v-else class="max-w-6xl mx-auto px-3 sm:px-4 py-4 space-y-4">
      <!-- Breadcrumb Bar -->
      <div class="bg-white rounded-md border border-gray-200/90 px-4 py-2.5 shadow-2xs text-xs font-normal text-gray-600 flex items-center gap-1.5 flex-wrap">
        <router-link to="/" class="hover:text-[#168039]">Home</router-link>
        <span class="text-gray-300">/</span>
        <router-link v-if="deal.categoryName" :to="`/products-list?category=${deal.categoryId}`" class="hover:text-[#168039]">{{ deal.categoryName }}</router-link>
        <span v-if="deal.categoryName" class="text-gray-300">/</span>
        <span class="text-gray-900 uppercase font-semibold">{{ deal.title }}</span>
      </div>

      <!-- Main Product Box -->
      <div class="bg-white rounded-md border border-gray-200/90 p-6 md:p-8 shadow-2xs">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <div class="md:col-span-4 flex flex-col items-center justify-center p-3 border border-gray-100 rounded-md bg-white">
            ...
            <div v-if="productImages.length > 1" class="flex gap-2 mt-4 overflow-x-auto max-w-full pb-1">
              <button
                v-for="(img, i) in productImages"
                :key="i"
                @click="activeImageIndex = i"
                class="shrink-0 w-12 h-12 rounded-md overflow-hidden border transition-all cursor-pointer"
                :class="activeImageIndex === i ? 'border-[#168039] ring-1 ring-[#168039]' : 'border-gray-200'"
              >
          ...
          <div class="md:col-span-8 space-y-4">
            <!-- Product Title -->
            <h1 class="text-2xl sm:text-3xl font-normal text-gray-900 leading-snug tracking-tight font-display">
              {{ deal.title }}
            </h1>
            ...
            <!-- Action Button / Quantity Bar (Synced with Cart) -->
            <div class="pt-1 w-fit">
              <!-- If item IS in cart: Show Quantity Controller Bar -->
              <div
                v-if="cartQty > 0"
                class="flex items-center bg-[#168039] text-white rounded-[4px] overflow-hidden shadow-2xs h-9 border border-[#146c30]"
              >
                <button
                  @click="handleDecrease"
                  aria-label="Decrease quantity"
                  class="px-3 h-full hover:bg-black/15 active:bg-black/25 transition-colors cursor-pointer flex items-center justify-center text-white"
                >
                  <Minus class="w-3.5 h-3.5 stroke-[2.5]" />
                </button>

                <span class="px-4 font-semibold text-sm leading-none select-none">{{ cartQty }}</span>
              ...
              <button
                ...
                class="h-9 px-4 border border-gray-300 hover:border-[#168039] text-gray-800 hover:text-[#168039] bg-white rounded-[4px] text-xs font-semibold flex items-center gap-2 transition-all shadow-2xs active:scale-[0.98] cursor-pointer"
              >
      ...
      <!-- Description & Return Policy Tabs Section -->
      <div class="bg-white rounded-md border border-gray-200/90 overflow-hidden shadow-2xs mt-6">
        <div class="bg-stone-100/70 border-b border-gray-200 px-4 pt-3 flex items-center gap-6">
          <button
            @click="activeTab = 'description'"
            class="pb-2 text-xs font-bold transition-all cursor-pointer"
            :class="activeTab === 'description' ? 'text-[#168039] border-b-2 border-[#168039]' : 'text-gray-600 hover:text-gray-900'"
          >
            {{ locale === 'bn' ? 'পণ্যের বিস্তারিত' : 'Description' }}
          </button>
          <button
            @click="activeTab = 'return'"
            class="pb-2 text-xs font-bold transition-all cursor-pointer"
            :class="activeTab === 'return' ? 'text-[#168039] border-b-2 border-[#168039]' : 'text-gray-600 hover:text-gray-900'"
          >
            {{ locale === 'bn' ? 'রিটার্ন পলিসি' : 'Return Policy' }}
          </button>
        </div>

        <div class="p-6 text-sm text-gray-700 leading-relaxed font-sans">
          <!-- Description Tab -->
          <div v-show="activeTab === 'description'" class="uppercase tracking-wide">
            <div v-if="deal.description" v-html="cleanedDescription" />
            <div v-else>{{ deal.title }}</div>
          </div>
          ...
          <div v-show="activeTab === 'return'">
            ...
            />
            <ul v-else class="list-disc pl-5 space-y-2 text-xs text-gray-600">
              <template v-if="locale === 'bn'">
                <li>পণ্য গ্রহণের সময় কোনো ত্রুটি থাকলে সর্বোচ্চ ৭ দিনের মধ্যে পরিবর্তন বা ফেরত নেওয়া যাবে।</li>
                <li>ব্যবহারের চিহ্নযুক্ত বা শারীরিকভাবে ক্ষতিগ্রস্ত পণ্য ফেরত বা পরিবর্তনের জন্য প্রযোজ্য হবে না।</li>
                <li>পণ্য ফেরতের সময় মূল বক্স, প্যাকেজিং এবং ক্যাশ মেমো থাকতে হবে।</li>
                <li>কোনো পণ্য পরিবর্তনের ক্ষেত্রে ডেলিভারি চার্জ প্রযোজ্য হতে পারে।</li>
              </template>
              <template v-else>
                <li>If the product is defective or damaged upon delivery, you can return or exchange it within 7 days.</li>
                <li>Products showing signs of use or physical damage are not eligible for return or exchange.</li>
                <li>Original box, packaging, and invoice must be returned along with the product.</li>
                <li>Delivery charges may apply for exchanges unless the return is due to our error.</li>
              </template>
            </ul>
          </div>
        </div>

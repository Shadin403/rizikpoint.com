...
class InvoiceController extends Controller
{
    ...
    public function bulk_invoice_download(\Illuminate\Http\Request $request)
    {
        $order_ids = $request->id;
        if (empty($order_ids)) {
            flash(translate('Please select at least one order'))->error();
        ...
        return PDF::loadView('backend.invoices.bulk_invoice',[
            'orders' => $orders,
            'font_family' => $font_family,
            'direction' => $direction,
            'text_align' => $text_align,
            'not_text_align' => $not_text_align
        ], [], [])->download('bulk-invoices-'.time().'.pdf');
    }
}

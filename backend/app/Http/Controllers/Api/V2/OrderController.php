...
class OrderController extends Controller
{
    public function store(Request $request, $set_paid = false)
    {
        ...
        // Dynamically calculate and save shipping cost on cart items before placing order
        $passed_shipping_cost = $request->input('shipping_cost');
        $shipping_per_item = $passed_shipping_cost !== null ? (doubleval($passed_shipping_cost) / count($cartItems)) : null;

        foreach ($cartItems as $key => $cartItem) {
            if ($cartItem->shipping_type == null || $cartItem->shipping_type == '') {
                $cartItem->shipping_type = 'home_delivery';
            }
            if ($cartItem->shipping_type == 'home_delivery') {
                $cartItem->shipping_cost = $shipping_per_item !== null ? $shipping_per_item : getShippingCost($cartItems, $key);
            } else {
        ...
        // Re-fetch cartItems with updated shipping costs
        $cartItems = Cart::where('user_id', $request->user_id)->get();
        ...
        foreach ($seller_products as $seller_product) {
            ...
            foreach ($seller_product as $cartItem) {
                ...
                $product_stock = $product->stocks->where('variant', $product_variation)->first() ?: $product->stocks->first();
